...
export default function Navbar() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-white/60 bg-cream/85 backdrop-blur-xl">
      <nav className="mx-auto flex h-20 max-w-7xl items-center justify-between gap-2 px-4 sm:px-6 lg:px-8">
        <Link href="/" aria-label="Emoura home" className="flex items-center gap-3">
          <BrandLogo compact />
        </Link>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            aria-label={t.search}
            onClick={openSearch}
            className="grid h-11 w-11 place-items-center rounded-full bg-white text-xl font-bold text-rosedark shadow-sm transition hover:bg-roselight"
          >
            ⌕
          </button>
          <button
            onClick={() => navTo("/favorites")}
            className="relative grid h-11 w-11 place-items-center rounded-full bg-white text-xl font-bold text-rosedark shadow-sm transition hover:bg-roselight"
          ...
          <button
            onClick={openCart}
            className="relative rounded-full bg-rosebrand px-3 py-3 text-sm font-extrabold text-white shadow-soft transition hover:bg-rosedark sm:px-4"
            aria-label={`${t.cart} (${cartCount})`}
          >
            <span className="hidden sm:inline">{t.cart}</span>
            <span className="sm:hidden">🛒</span>
            {cartCount > 0 && (
              <span className="absolute -end-2 -top-2 grid h-6 min-w-6 place-items-center rounded-full bg-gold px-1 text-xs font-black text-rosedark">
                {cartCount}
              </span>
            )}
          </button>
